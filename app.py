...
class Calculator:
    ...
    def __init__(self, master):
        ...
        self.createButton()
    #creates each button with the value we want.
    def createButton(self):
        ...
        #Button list for calculator rows
        row1=[b7,b8,b9,b_add]
        row2=[b4,b5,b6,b_sub]
        row3=[b1,b2,b3,b_mult]
        row4=[b_clear,b0,b_equal,b_div]
        #assigns buttons to their location on the gui
        r=1
    ...
    def clickButton(self,value):
        ...
        #if the user clicked "c" then clear the line.
        if value =='c':
            self.equation.delete(-1, END)
        #if user clickes = then compute and display
        elif value == '=':
            answer = str(eval(current_equation))
            self.equation.delete(-1, END)
            self.equation.insert(0, answer)
        #if user clicked any other butten add it to the line.
        else:
